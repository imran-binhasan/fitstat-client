...
const TrainerDetails = () => {
  ...
  return (
    <div className="container mx-auto py-5">
      <PageTitle
        title={`${name} Details`}
        subTitle="Learn about your trainer before booking"
      />
      ...
      <div className="grid grid-cols-1 md:grid-cols-2">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg p-6 flex flex-col md:flex-row items-center gap-6">
          {/* Image */}
          <img
            className="w-56 h-56 md:w-72 md:h-72 rounded-full border shadow-md object-cover"
            src={photoURL}
            alt={name}
          />
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg p-6">
          ...
          <div className="flex flex-wrap gap-3 mt-3 text-gray-700 dark:text-gray-300">
            <p className="flex items-center gap-1 border px-4 py-1 rounded-md">
              <HiOutlineArrowCircleRight /> {age} Years Old
            </p>
            <p className="flex items-center gap-1 border px-4 py-1 rounded-md">
              <MdWorkOutline /> {experience} Years Experience
            </p>
          </div>
          ...
          <div className="flex flex-wrap gap-2 mt-4">
            {skills?.map((each, index) => (
              <span
                className="border px-4 py-1 rounded-md text-sm bg-gray-100 dark:bg-gray-700"
                key={index}
              >
                {each}
              </span>
