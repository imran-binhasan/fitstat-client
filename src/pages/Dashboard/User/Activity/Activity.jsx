        ...
        <table className="w-full bg-white border border-gray-200 shadow-lg rounded-lg overflow-hidden">
          ...
          <tbody>
              ...
              >
                ...
                <td className={`py-2 px-2 flex items-center gap-2 space-y-1 space-x-2 uppercase ${status === 'rejected'?'text-red-500':'text-yellow-500'}`}>
                  {status} {status === 'rejected' && <FaEye onClick={()=> setModalIsOpen(true)}/>}
                </td>
              </tr>
            
          </tbody>
        </table>
      </div></>:<div className="flex justify-center h-[60vh] items-center">
        <p>You don't have any activities !</p></div>}
      {isModalOpen && (
