...
const Classes = () => {
    ...
    const { data: classes, total } = useClasses(search, page, limit);
    ...
    return (
        <div className="container mx-auto py-5">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6"> 
                {classes?.map((each) => (
                    <div key={each._id} className="border rounded-lg overflow-hidden shadow-sm hover:shadow transition-shadow">
                        <div className="aspect-[4/3] w-full">
                            <img src={each.image} alt={each.name} className="w-full h-full object-cover" />
                        </div>
                        <div className="p-3 space-y-2">
                            <h3 className="text-lg font-medium text-gray-800">{each.name}</h3>
